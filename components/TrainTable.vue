<template>
  <div class="d-flex mb-3" style="gap: 0.5rem">
    <button :class="`btn ${type == 1 ? 'btn-primary' : 'btn-outline-primary'}`" @click="type = 1">Origin</button>
    <button :class="`btn ${type == 2 ? 'btn-primary' : 'btn-outline-primary'}`" @click="type = 2">Bobot</button>
    <button :class="`btn ${type == 3 ? 'btn-primary' : 'btn-outline-primary'}`" @click="type = 3">Normalisasi</button>
  </div>
  ...
  <client-only>
    <TrainImportModal @imported="onImported" />
  </client-only>
</template>
...
const getData = async () => {
  try {
    const res: any = await fetchAPI('/data/train')
    data.value = res.data;
